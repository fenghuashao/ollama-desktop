...
export default function StatusPage(): React.ReactElement {
  const { status, isProcessRunning, checkStatus, startService, stopService } = useStatusStore()
  ...
  const getCompatibilityIcon = (level: string): React.ReactElement => {
    switch (level) {
      case 'compatible':
        ...
        <div className="p-6 bg-card border rounded-xl shadow-sm">
          <div className="flex items-center gap-3 mb-4">
            <div className={`p-2 rounded-lg ${status.isRunning ? 'bg-green-100' : 'bg-red-100'}`}>
              <Server
                className={`w-6 h-6 ${status.isRunning ? 'text-green-600' : 'text-red-600'}`}
              />
            </div>
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-center py-2 border-b">
              ...
              <span
                className={`font-medium ${status.isRunning ? 'text-green-600' : 'text-red-600'}`}
              >
            ...
            <div className="flex justify-between items-center py-2">
              ...
              <span className="text-sm">{new Date(status.lastChecked).toLocaleTimeString()}</span>
            </div>
            ...
            <div className="pt-2">
              ...
              <div className="flex flex-wrap gap-2">
                {status.supportedFeatures.map((feature) => (
                  <span
                    key={feature}
                    className="text-xs bg-secondary px-2 py-1 rounded-full border"
                  >
                    {feature}
                  </span>
                ))}
