...
app.whenReady().then(() => {
  ...
  ipcMain.handle('ollama:get-version', async () => {
    try {
      const { stdout } = await execAsync('ollama --version')
      // Output format is typically "ollama version is 0.1.27"
      const match = stdout.match(/version is (\d+\.\d+\.\d+)/)
      return match ? match[1] : 'unknown'
    } catch (error) {
      console.warn('Failed to get ollama version via CLI:', error)
      return 'unknown'
    }
  })
